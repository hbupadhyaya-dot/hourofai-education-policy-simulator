                            ...
                            <div className="relative">
                              ...
                              >
                                <span className="font-bold text-purple-600 bg-purple-50 px-2 py-1 rounded text-xs whitespace-nowrap">
                                  {policy.id === 'COMM_INPUT' ? `${Math.round((policyIntensities[policy.id] || 50) * 0.08)} meetings/mo` :
                                   policy.id === 'IMPACT_REP_STD' ? `${policyIntensities[policy.id] || 50}` :
                                   policy.id === 'LOCAL_JOB_ALIGN' ? `${Math.round((policyIntensities[policy.id] || 50) * 2)} hrs/yr` :
                                   `${policyIntensities[policy.id] || 50}`}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="relative">
                              ...
                              >
                                <span className="font-bold text-orange-600 bg-orange-50 px-2 py-1 rounded text-xs whitespace-nowrap">
                                  {policy.id === 'INTEROP_STD' ? `${policyIntensities[policy.id] || 50}` :
                                   policy.id === 'INFRA_INVEST' ? `${Math.round((policyIntensities[policy.id] || 50) * 0.5)}%` :
                                   policy.id === 'ACCESS_STD' ? `${50 + Math.round((policyIntensities[policy.id] || 50) * 0.5)}%` :
                                   `${policyIntensities[policy.id] || 50}`}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="relative">
                              ...
                              >
                                <span className="font-bold text-red-600 bg-red-50 px-2 py-1 rounded text-xs whitespace-nowrap">
                                  {policy.id === 'STATE_FED_PART' ? `$${Math.round((policyIntensities[policy.id] || 50) * 1)}M` :
                                   policy.id === 'INNOV_SANDBOX' ? `${policyIntensities[policy.id] || 50}` :
                                   policy.id === 'MODEL_EVAL_STD' ? `${policyIntensities[policy.id] || 50}` :
                                   `${policyIntensities[policy.id] || 50}`}
                                </span>
                              </div>
                            </div>
